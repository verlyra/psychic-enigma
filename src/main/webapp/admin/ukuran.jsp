...
<%
    ...
    if("add".equals(action) && "POST".equalsIgnoreCase(request.getMethod())) {
        try {
            ...
            String sql = "INSERT INTO master_ukuran (nama_ukuran) VALUES (?)";
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, request.getParameter("nama_ukuran"));
            pstmt.executeUpdate();
            pstmt.close();
            conn.close();
            response.sendRedirect("ukuran.jsp");
            return;
        ...
        try {
            koneksi k = new koneksi();
            Connection conn = k.bukaKoneksi();
            String sql = "UPDATE master_ukuran SET nama_ukuran=? WHERE id=?";
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, request.getParameter("nama_ukuran"));
        ...
        try {
            koneksi k = new koneksi();
            Connection conn = k.bukaKoneksi();
            PreparedStatement pstmt = conn.prepareStatement("DELETE FROM master_ukuran WHERE id = ?");
...
<head>
    <meta charset="UTF-8">
    <title>Master Ukuran - VEND.IO Admin</title>
...
<body class="flex min-h-screen">
    ...
    <aside class="w-64 bg-black text-white flex flex-col justify-between py-6">
        <div>
            ...
            <nav class="flex flex-col space-y-2">
                ...
                <a href="ukuran.jsp" class="px-6 py-4 font-[900] tracking-wider uppercase bg-[#FACC15] text-black">MASTER UKURAN</a>
    ...
    <main class="flex-1 p-10 flex flex-col relative">
        <h2 class="text-4xl font-[900] text-black tracking-tight uppercase mb-8">SIZE / UNITS</h2>
        ...
        <div class="bg-black p-4 w-full brutal-shadow flex-1 flex flex-col">
            <div class="bg-white border-[4px] border-black flex-1 p-6 relative flex flex-col">
                <h3 class="font-[900] text-2xl uppercase tracking-tighter mb-6 text-white bg-black inline-block relative -top-12 -left-2 px-4 pb-0 pt-2">MASTER UKURAN</h3>
                ...
                <table class="w-full text-left -mt-8 mb-auto">
                    <thead>
                        <tr class="border-b-[4px] border-black">
                            ...
                            <th class="py-3 font-[900] text-black uppercase tracking-wider">SIZE / UNIT NAME</th>
                    ...
                    <tbody>
                        <%
                            try {
                                ...
                                ResultSet rs = stmt.executeQuery("SELECT * FROM master_ukuran ORDER BY id DESC");
                                while(rs.next()) {
                        %>
                        <tr class="font-bold border-b border-gray-200">
                            <td class="py-4"><%= rs.getInt("id") %></td>
                            <td class="py-4 uppercase"><%= rs.getString("nama_ukuran") %></td>
                            <td class="py-4 text-right whitespace-nowrap">
                                <a href="javascript:void(0)" onclick="openEditUkuran(<%= rs.getInt("id") %>, '<%= rs.getString("nama_ukuran").replace("'", "\\'") %>')" class="text-[#3498DB] font-[900] uppercase text-sm">EDIT</a>
                                <span class="text-black font-black mx-1">|</span>
                                <a href="ukuran.jsp?action=delete&id=<%= rs.getInt("id") %>" class="text-red-500 hover:text-red-700 font-[900] uppercase text-sm" onclick="return confirm('Hapus ukuran ini?');">DELETE</a>
                            </td>
                        </tr>
                        <%      }
                ...
                <div class="flex justify-end mt-8">
                    <button onclick="document.getElementById('addModal').classList.remove('hidden')" class="bg-[#A855F7] border-[4px] border-black px-6 py-3 font-[900] text-white tracking-wider uppercase brutal-btn-shadow hover:bg-purple-600 transition-colors">
                        + ADD SIZE / UNIT
                    </button>
                </div>
            </div>
    ...
    <div id="addModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
        ...
        <div class="bg-black p-3 w-full max-w-lg brutal-shadow">
            <div class="bg-white border-[4px] border-black p-8 relative">
                <h3 class="font-[900] text-2xl uppercase tracking-tighter mb-6 text-white bg-black inline-block absolute -top-8 -left-3 px-4 py-2">ADD NEW SIZE</h3>
                
                <form action="ukuran.jsp?action=add" method="POST" class="grid grid-cols-1 gap-6 mt-6">
                    <div>
                        <label class="block font-[900] text-black tracking-wider uppercase mb-1">SIZE / UNIT NAME (e.g. Kg, Pcs, XL)</label>
                        <input type="text" name="nama_ukuran" required class="w-full border-[4px] border-black p-3 font-bold text-black focus:outline-none">
                    </div>
    ...
    <div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
        ...
        <div class="bg-black p-3 w-full max-w-lg brutal-shadow">
            <div class="bg-white border-[4px] border-black p-8 relative">
                <h3 class="font-[900] text-2xl uppercase tracking-tighter mb-6 text-white bg-black inline-block absolute -top-8 -left-3 px-4 py-2">EDIT SIZE</h3>
                
                <form action="ukuran.jsp?action=edit" method="POST" class="grid grid-cols-1 gap-6 mt-6">
                    <input type="hidden" name="id" id="edit_id">
                    <div>
                        <label class="block font-[900] text-black tracking-wider uppercase mb-1">SIZE / UNIT NAME</label>
                        <input type="text" name="nama_ukuran" id="edit_nama" required class="w-full border-[4px] border-black p-3 font-bold text-black focus:outline-none">
                    </div>
    ...
    <script>
        function openEditUkuran(id, nama) {
